"""
this programm transforms vocabulary from the end of the Englsih Book and make it to dictionary : russianWord : englsihTranslation 
For exmaple :

input:
    # (live) next door [neksdn‫]ޝ‬ – соседский, по соседству / суседскі,па суседству
    # (un)fairly [(¤¡n)¥feli] (adv.) – (не)справедливо / (не)справядліва
    # at first sight [sa+t] – с первого взгляда / з першага погляду

output:
    {
        "соседский, по соседству": [
            "(live) next door",
            "next door"
        ],
        "(не)справедливо": [
...
! Put Our input in inputTransformationFile.txt
"""

import json 
import re 

OUT_FILE   = "outputVocabularyTransformation.json"
INPUT_FILE = "inputTransformationFile.txt"


def getText(file=INPUT_FILE):
